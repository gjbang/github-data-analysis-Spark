...
def release_id(s):
    map = json.loads(s)
    return map['id']
get_release_idUDF = F.udf(release_id, StringType())
time_transformUDF = F.udf(time_transform, StringType())

inserts = read_df.filter("type = 'ReleaseEvent'")
...
inserts = inserts.withColumn("payload_release_id", get_release_idUDF(inserts.payload["release"]))
...
s = spark.sql("show tables in default like 'releaseTable'")
flag = len(s.collect())
if flag:
    print("exist")
    spark.sql("insert into default.releaseTable select * from tmptable")
else:
    print("not exist")
    spark.sql("create table IF NOT EXISTS default.releaseTable select * from tmptable")
