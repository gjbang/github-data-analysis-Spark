...
inserts = read_df.filter("type = 'CreateEvent'")
...
inserts = inserts.withColumn("payload_master_branch", inserts.payload["master_branch"])
inserts = inserts.withColumn("payload_description", inserts.payload["description"])
...
s = spark.sql("show tables in default like 'createTable'")
flag = len(s.collect())
if flag:
    print("exist")
    spark.sql("insert into default.createTable select * from tmptable")
else:
    print("not exist")
    spark.sql("create table IF NOT EXISTS default.createTable select * from tmptable")
