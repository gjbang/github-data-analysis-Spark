...
inserts = read_df.filter("type = 'WatchEvent'")
...
s = spark.sql("show tables in default like 'watchTable'")
flag = len(s.collect())
if flag:
    print("exist")
    spark.sql("insert into default.watchTable select * from tmptable")
else:
    print("not exist")
    spark.sql("create table IF NOT EXISTS default.watchTable select * from tmptable")
